...
function Model!(dy,y,par,t)
  ...
  k11=1E5 #PR8, RIGI is assumed antagonized
  n=3
  RIGI=1
  k12=par[1]
  k13=par[2]
...
stop=1 #Tesla Plot stop

solMatrix = fill(0.0,(stop-start+1),7,length(tgraph))

for i=start:stop
  p=[9.746083465,	12.51102862,	13.56171372,	10.38525354,	3.480831989,	45.92181719,	5.463818728,	0.068040206,	0.115073479,	1.052601121,	0.202073596,	0.635289003,	1.537303005,	47.88332354,	0.197042272]
  sol = solve(ProbMaker(Model!,u0,tspan,p),alg)
  solMatrix[(i-start+1),:,:]=sol(tgraph)[:,:]
  #EXported PARAMeters, LONG/SHORT/BEST version
  CSV.write("exparamrigi.csv",DataFrame(solMatrix[(i-start+1),:,:]),append=false)
end
